# teamagent — Research note: CLIProxyAPI (2026-06-13)

Deep-dive on [router-for-me/CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) (Go, MIT,
~37k★, latest v7.1.x). Purpose: it is the **generalized superset** of what teamagent does narrowly.
Mining its proven internal factoring for teamagent's roadmap milestones — **without** adopting its
generality, which is exactly what teamagent's non-goals refuse. Companion to [03-research-notes.md].

## What it is (and how it differs from teamagent)
...
So: **teamagent is the opinionated path through the map that CLIProxyAPI draws in full.** The value
...
joined by a routing layer. teamagent has all three **embryonically but fused** (`provider/` trait
...
**The teamagent mapping is unusually clean** because inbound is *always* Anthropic Messages: every
backend needs exactly `(anthropic → X)` request + `(X → anthropic)` response. Adding Gemini =
register two transforms + one executor, **touching zero of `forward.rs`**. teamagent's existing
...
| Idea (from CLIProxyAPI) | Serves teamagent milestone | On-thesis? |
|---|---|---|
...
| **`session-affinity`** key extraction (`X-Session-ID`, `metadata.user_id`, `X-Amp-Thread-Id`) | only if teamagent serves **N concurrent Claude Code windows** through one daemon, each pinned to its own account | 🔵 future-conditional |
| **`fill-first` strategy** | alternative to round-robin; teamagent's "soonest 7d-reset first" ranking is already a smarter fill-first variant — confirms the instinct | ✅ already have better |
...
| **Management API gate** (`allow-remote`, hashed `secret-key`, `disable-control-panel`) | richer version of teamagent's loopback-exempt `/teamagent/*` gate — only if a remote/web panel is ever wanted | 🔵 low priority (TUI-first identity) |

## Studied and deliberately rejected (the discipline line)

- **Multi-inbound-format** (accepting OpenAI/Gemini *clients*). This dissolves teamagent's identity —
  the bet is **one** harness. CLIProxyAPI is gateway-for-everything; teamagent is harness-preserver.
...
keep refusing the **generality** — the generality is precisely what teamagent's non-goals exist to
reject.
