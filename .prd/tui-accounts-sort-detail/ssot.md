...
| 같은 그룹에서도 어카운트 이름으로 정렬 (1 이름 정렬) | Sort mode **name**: within a group, rows ordered by account name (case-insensitive, then config index as a stable tiebreak). Default mode. | unit: `display_order_by_name_sorts_within_group` |
