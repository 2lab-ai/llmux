...
| 3 | Sort mode `name` (default): within a group, account name ascending, case-insensitive and NATURAL — digit runs compare as numbers, so `ai` < `ai1` < `ai2` < `ai10` (the empty number slot sorts first); the lowercased spelling (`ai01` vs `ai1`) then the config index are the tiebreaks. | `triage::natural_key`, `triage.rs` |
