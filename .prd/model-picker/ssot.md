...
| 모델 카탈로그 변경해서 | `llmux run` injects a `modelPicker` lineup built from `GET /llmux/models` via `claude --settings` | pending code + unit tests |
| 지원하는 모델 출력 | `/model` inside a `llmux run` session lists the llmux catalog rows (screen capture) and a selected non-Claude row routes to that backend (activity log line) | pending live receipt |
