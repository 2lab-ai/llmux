...
pub(crate) struct EventDeadline {
...
/// then simply does not render (never a crash). std has no timezone database
/// and chrono is too heavy for one offset, so this is a small hand-rolled
/// parser paired with the civil-date math already in this module.
pub(crate) fn parse_event_deadline(s: &str) -> Option<EventDeadline> {
...
pub(crate) fn zone_label(offset_secs: i64) -> String {
    match offset_secs {
        -25_200 => "PT".to_string(),
        -14_400 => "ET".to_string(),
        0 => "UTC".to_string(),
...
mod tests {
    ...
    // ---- event banner: offset → zone label ----
    ...
    // ---- event banner: RFC3339 deadline parsing ----
