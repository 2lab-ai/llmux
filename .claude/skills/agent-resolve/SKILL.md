...
merge to `main` and does **not** 배포. In this repo "배포" pushes to a **public** preview channel
(main → CI preview → brew); that recovery cost is asymmetric, so **a human gates merge + deploy**.
...
2. **Lock it + isolate in a worktree.** Add the `agent-wip` label (prevents `agent-loop`/double-work).
   ...
   then `cd` into `<repo>/../llmux-wt/issue-<N>`. (`-B` resets that path's branch to current `main`.)
...
6. **Kill-switches (hard — enforce, don't rationalize past them):**
   ...
   - **never** commit to / push `main` (feature branch only).
...
- **Create/reset:** `git -C <repo> worktree add -B agent/issue-<N>-<slug> "<repo>/../llmux-wt/issue-<N>" main`.
- **A branch can be checked out in only one worktree.** If `git worktree list` already shows one for
  this issue, reuse it (`cd` in, `git pull`/rebase onto `main`) instead of creating a second.
...
- Touching `main`, `Cargo.toml` version, tags, workflows, or secrets.
